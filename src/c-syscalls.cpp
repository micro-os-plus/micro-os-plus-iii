...
#pragma GCC diagnostic ignored "-Wmissing-declarations"
  ...
  int __attribute__((weak))
  _execve_r (const char* path, char* const argv[], char* const envp[])
  {
    return __posix_execve (path, argv, envp);
  }

  int __attribute__((weak))
  _fcntl_r (int fildes, int cmd, int arg)
  {
  ...
  int __attribute__((weak))
  _fstat_r (int fildes, struct stat* buf)
  {
    return __posix_fstat (fildes, buf);
  }

  int __attribute__((weak))
  _gettimeofday_r (struct timeval* ptimeval, void* ptimezone)
  {
    return __posix_gettimeofday (ptimeval, ptimezone);
  }

  int __attribute__((weak))
  _isatty_r (int fildes)
  {
    return __posix_isatty (fildes);
  }

  int __attribute__((weak))
  _kill_r (pid_t pid, int sig)
  {
    return __posix_kill (pid, sig);
  }

  int __attribute__((weak))
  _link_r (const char* existing, const char* _new)
  {
    return __posix_link (existing, _new);
  }

  off_t __attribute__((weak))
  _lseek_r (int fildes, off_t offset, int whence)
  {
    return __posix_lseek (fildes, offset, whence);
  }

  int __attribute__((weak))
  _mkdir_r (const char* path, mode_t mode)
  {
    return __posix_mkdir (path, mode);
  }

  int __attribute__((weak))
  _open_r (const char* path, int oflag, int mode)
  {
    return __posix_open (path, oflag, mode);
  }

  ssize_t __attribute__((weak))
  _read_r (int fildes, void* buf, size_t nbyte)
  {
    return __posix_read (fildes, buf, nbyte);
  }

  int __attribute__((weak))
  _rename_r (const char* oldfn, const char* newfn)
  {
    return __posix_rename (oldfn, newfn);
  }

  int __attribute__((weak))
  _stat_r (const char* path, struct stat* buf)
  {
    return __posix_stat (path, buf);
  }

  clock_t __attribute__((weak))
  _times_r (struct tms* buf)
  {
  ...
  pid_t __attribute__((weak))
  _wait_r (int* stat_loc)
  {
    return __posix_wait (stat_loc);
  }

  ssize_t __attribute__((weak))
  _write_r (int fildes, const void* buf, size_t nbyte)
  {

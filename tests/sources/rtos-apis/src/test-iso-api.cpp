...
test_iso_api (bool extra)
{
  ...
  printf ("\n%s - Threads\n", test_name);
  {
...
#if 1
    // TODO: check thread termination; at a certain moment some user faults
    // were triggered.
    {
      char c;

      estd::thread th21{ task2, &c };

      estd::thread th31{ task3, &c };

      estd::thread th41{ task4, 7, "xyz" };
...

    {
      ...
      std::thread th22{ task2, &c };

      std::thread th32{ task3, &c };

      std::thread th42{ task4, 7, "xyz" };
    ...
    if (extra)
      {
        ...
        estd::mutex mxl;
        std::unique_lock<estd::mutex> lock{ mxl };

        cv11.wait (lock);

        auto pred = [] () { return is_ready (); };
...
#pragma GCC diagnostic ignored "-Wuseless-cast"
  ...
  estd::this_thread::sleep_until (estd::chrono::realtime_clock::now ()
                                  + 100ms);
  ...
  if (extra)
    {
      ...
      estd::this_thread::sleep_until (estd::chrono::systick_clock::now ()
                                      + 1s);
...
#pragma GCC diagnostic pop
  ...
  // std::chrono::system_clock::now() + 1000ms; this_thread::sleep_until

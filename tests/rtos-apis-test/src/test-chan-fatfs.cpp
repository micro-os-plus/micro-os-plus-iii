...
int
test_diskio (
    posix::block_device& pdrv, /* Physical drive number to be checked (all data
...
test_chan_fatfs (bool extra __attribute__ ((unused)))
{
  ...
  for (int i = 0; i < 3; ++i)
    {
      ...
      // >128 & <4085 -> FAT12
      my_block* chbk = new my_block{ "ch-bk-1", 512u, 512u, 128u };
      ...
      posix::chan_fatfs_file_system* fs
          = new posix::chan_fatfs_file_system{ "fat-1", *chbk };
...

  {
    ...
    // <4085 -> FAT12
    my_block* chbk = new my_block{ "ch-bk-1m", 512u, 512u, 128u };
    ...
    posix::chan_fatfs_file_system_lockable<rtos::mutex>* fs
        = new posix::chan_fatfs_file_system_lockable<rtos::mutex>{ "fat-1m",
                                                                   *chbk,
                                                                   chmx };
...
// so can be executed only on the synthetic platform.
#if !defined(OS_IS_CROSS_BUILD)
  {
    ...
    // >4085 && <65535 -> FAT16
    my_block* chbk = new my_block{ "ch-bk-2", 512u, 512u, 8229u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-2", *chbk };
...

  {
    ...
    // >65535 -> FAT32
    my_block* chbk = new my_block{ "ch-bk-3", 512u, 512u, 66130u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-3", *chbk };
...

  {
    ...
    my_block* chbk = new my_block{ "ch-bk-4", 512u, 512u, 4096u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-4", *chbk };
...

  {
    ...
    // >128 & <4085 -> FAT12
    my_block* chbk = new my_block{ "ch-bk-5", 1024u, 1024u, 128u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-5", *chbk };
...

  {
    ...
    // >128 & <4085 -> FAT12
    my_block* chbk = new my_block{ "ch-bk-6", 4096u, 4096u, 128u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-6", *chbk };
...

  {
    ...
    // >128 & <4085 -> FAT12
    my_block* chbk = new my_block{ "ch-bk-7", 512u, 4096u, 128u };
    ...
    posix::chan_fatfs_file_system* fs
        = new posix::chan_fatfs_file_system{ "fat-7", *chbk };
...
int
test_diskio (
    posix::block_device& bd, /* Physical drive number to be checked (all data
                                on the drive will be lost) */
    std::size_t ncyc, /* Number of test cycles */
    uint8_t* buf, /* Pointer to the working buffer */
    std::size_t sz_buff /* Size of the working buffer in unit of byte */

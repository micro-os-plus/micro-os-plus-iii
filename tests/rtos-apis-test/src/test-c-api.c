  ...
  while (it != end)
    {
      ...
      unsigned int used
          = (unsigned int)(os_thread_stack_get_size (pst)
                           - os_thread_stack_get_available (pst));
      unsigned int used_proc
          = (unsigned int)(used * 100 / os_thread_stack_get_size (pst));
...
#pragma GCC diagnostic pop
...

  {
    ...
    os_evflags_wait (&ev1, 0x1, NULL, os_flags_mode_all | os_flags_mode_clear);

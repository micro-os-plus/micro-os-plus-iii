...
#pragma GCC diagnostic pop
    ...
    public:
      ...
      virtual int
      bind (const /* struct */ sockaddr* address,
            socklen_t address_len) override;

      virtual int
      connect (const /* struct */ sockaddr* address,
               socklen_t address_len) override;

      virtual int
      getpeername (/* struct */ sockaddr* address,
                   socklen_t* address_len) override;

      virtual int
      getsockname (/* struct */ sockaddr* address,
                   socklen_t* address_len) override;
...
namespace os
{
  namespace posix
  {
    ...
    template <typename T, typename L>
    ssize_t
    socket_lockable<T, L>::sendmsg (const /* struct */ msghdr* message,
                                    int flags)
    {
      std::lock_guard<L> lock{ locker_ };
